...
void testCamera() {
	...
	// ------------------------------------
	Shader shaderProgram("shaders/box/vertShader.glsl", "shaders/box/fragShader.glsl");
	...
	// Charaistic 1(Texture Coordinate) 存入VAO
...
glm::mat4 lookAt(glm::vec3 cameraPos, glm::vec3 cameraTarget , glm::vec3 up) {
	glm::vec3 cameraDirection = glm::normalize(cameraPos - cameraTarget);
	glm::vec3 cameraRight = glm::normalize(glm::cross(up, cameraDirection));
	if (glm::length(cameraRight) < 1e-6f) // 设置一个小的阈值
	{
		// 如果平行，使用备用上向量（例如世界空间的z轴正向）
		cameraRight = glm::vec3(0.0f, 0.0f, 1.0f);
	}
	glm::vec3 cameraUp = glm::cross(cameraDirection, cameraRight);

	// 构造视图矩阵（列主序）
	glm::mat4 viewMatrix(1.0f); // 初始化为单位矩阵

	// 设置第一列 (R)
	viewMatrix[0][0] = cameraRight.x; // row0 col0
	viewMatrix[0][1] = cameraRight.y; // row1 col0
	viewMatrix[0][2] = cameraRight.z; // row2 col0
	viewMatrix[0][3] = 0.0f; // row3 col0

	// 设置第二列 (U)
	viewMatrix[1][0] = cameraUp.x; // row0 col1
	viewMatrix[1][1] = cameraUp.y; // row1 col1
	viewMatrix[1][2] = cameraUp.z; // row2 col1
	viewMatrix[1][3] = 0.0f; // row3 col1

	// 设置第三列 (-D)
	viewMatrix[2][0] = -cameraDirection.x; // row0 col2
	viewMatrix[2][1] = -cameraDirection.y; // row1 col2
	viewMatrix[2][2] = -cameraDirection.z; // row2 col2
	viewMatrix[2][3] = 0.0f; // row3 col2

	// 设置第四列 (平移)
	viewMatrix[3][0] = -glm::dot(cameraRight, cameraPos); // row0 col3
	viewMatrix[3][1] = -glm::dot(cameraUp, cameraPos); // row1 col3
	viewMatrix[3][2] = glm::dot(cameraDirection, cameraPos); // row2 col3
//	viewMatrix[3][3] = 1.0f; // row3 col3
...
void test3D() {
	...
	// build and compile our shader program
	// ------------------------------------
	Shader shaderProgram("shaders/3D/vertShader.glsl", "shaders/3D/fragShader.glsl");
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		projection = glm::perspective(glm::radians(45.0f), (float)SCR_WIDTH / SCR_HEIGHT, 10.0f, 100.0f);
		shaderProgram.setMat4("projection", projection);
		//glDrawElements(GL_TRIANGLES, 6, GL_UNSIGNED_INT, 0);
		// glBindVertexArray(0); // no need to unbind it every time 
...
void testGLM() {
	...
	// ------------------------------------
	// vertex shader
	unsigned int vertexShader = loadShader(GL_VERTEX_SHADER, "shaders/matrix/vertShader.glsl");

	// fragment shader
	unsigned int fragmentShader = loadShader(GL_FRAGMENT_SHADER, "shaders/matrix/fragShader.glsl");
